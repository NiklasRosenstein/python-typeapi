...
import typeapi
from typeapi.model import Annotated, ForwardRef, Type, TypeVar, eval_types, infuse_type_parameters
...
class MyGeneric1(t.Generic[T]):
    pass


class MyGeneric2(MyGeneric1[int], t.Generic[T]):
    pass


class MyGeneric3(t.Generic[T], MyGeneric2[T]):
    pass
...
def test_type_of_special_generic():
    ...
    assert Type.of(t.Mapping[str, int]) == Type(
    ...
    assert Type.of(t.Mapping[K, V]) == Type(
        collections.abc.Mapping,
        2,
        None,
        (
...
def test_base_type_parameters():
    ...
    class MyGeneric4(MyGeneric1[K], t.Generic[K, V]):
        pass

    class MyGeneric5(MyGeneric4[str, V]):
        pass
...
def test_ForwardRef_evaluate():
    ...
    if sys.version_info[:3] > (3, 9, 6):
        # mypy doesn't seem to understand comparing version_info against a three-part version
        ref = ForwardRef(t.ForwardRef("T", module=__name__))  # type: ignore[call-arg]
